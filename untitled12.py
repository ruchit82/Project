...
st.title("📊 Export Sales and Monthly Sales Analysis Dashboard")

# Upload Excel file
uploaded_file = st.file_uploader("Upload the Export Sales or Monthly Sales Excel File", type=['xlsx', 'xls'])
...
if uploaded_file:
    ...
    # Check columns to differentiate between files
    if 'PARTY' in data.columns and 'TYPE' in data.columns and 'SIZE' in data.columns:
        # This seems like Export Sales File
        st.write("### Data Information - Export Sales File")
    ...
    elif 'MONTH' in data.columns and 'SALES' in data.columns:
        # This seems like Monthly Sales File
        st.write("### Data Information - Monthly Sales File")
        ...
        # Unique counts for categorical columns
        st.write("### Unique Values in Categorical Columns")
        st.write(f"Unique Months: {data['MONTH'].nunique()}")
        st.write(f"Unique Sales Types: {data['SALES_TYPE'].nunique()}")

        # Time-based Analysis
        st.write("### Sales Over Time")
        data['DATE'] = pd.to_datetime(data['DATE'])
        sales_summary = data.groupby('DATE').agg({'SALES': 'sum'}).reset_index()
        fig1, ax1 = plt.subplots(figsize=(10, 6))
        sns.lineplot(x='DATE', y='SALES', data=sales_summary, marker='o', label='Sales')
        ...
        month_summary = data.groupby('MONTH').agg({'SALES': 'sum'}).reset_index()
        fig2, ax2 = plt.subplots(figsize=(8, 6))
        sns.barplot(x='MONTH', y='SALES', data=month_summary, palette='coolwarm')
